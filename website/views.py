...
def get_completion_code(ip):
    ...
    if code == "GB":
        completion_code = "https://www.prolific.ac/submissions/complete?cc=2K6AAHMN"
    if code == "CA":
        completion_code = "https://www.prolific.ac/submissions/complete?cc=XWBNFJPH"
    elif code == "AU":
        completion_code = "https://www.prolific.ac/submissions/complete?cc=MVZRCQ2H"
    elif code == "FR":
        completion_code = "https://www.prolific.ac/submissions/complete?cc=EHYL8LFW"
    elif code == "DE":
        completion_code = "https://www.prolific.ac/submissions/complete?cc=CM97W42K"
    elif code == "NL":
         completion_code = "https://www.prolific.ac/submissions/complete?cc=LHHXIDH3"
    elif code == "IT":
         completion_code = "https://www.prolific.ac/submissions/complete?cc=S5F2AMZA"
    elif code == "PL":
         completion_code = "https://www.prolific.ac/submissions/complete?cc=A2JH8BCU"
    elif code == "TR":
         completion_code = "https://www.prolific.ac/submissions/complete?cc=IAQY2UVN"
    else:
...
###### new filtering
    #(1) on country
    if current_country == "AU":                     # check if user is in a selected country
        country_not_ok = False
    elif current_country == "CA":
        country_not_ok = False
    elif current_country == "GB":
         country_not_ok = False
    ...
    if Submission.objects.filter(asn=current_ASN).count() >= dt_point or Submission.objects.filter(ip_prefix=prefix, asn=current_ASN).exists():
        return redirect('fail')
